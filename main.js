...
class DXBallGame {
  constructor() {
    ...
    for (let col = 0; col < 9; col++) {
      for (let row = 0; row < 5; row++) {
        ...
        const brickX =
          col * (this.brick.width + this.brick.padding) + this.brick.offsetLeft;
        const brickY =
          row * (this.brick.height + this.brick.padding) + this.brick.offsetTop;
  ...
  draw() {
    ...
    //если мяч не попал в платформу - перезагружаем страницу, иначе - игра продолжается
